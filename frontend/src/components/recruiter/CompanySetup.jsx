...
const CompanySetup = () => {
  ...
  // Pre-fill the form with company data when the component mounts or when the company data changes
  useEffect(() => {
    // Use the companyName from location state if available
    const defaultCompanyName =
      location.state?.companyName || singleCompany?.name || "";

    if (singleCompany) {
      setInput({
        name: defaultCompanyName, // Set the company name from state or singleCompany
        description: singleCompany.description || "",
        website: singleCompany.website || "",
        location: singleCompany.location || "",
        file: null, // Reset file since it's a new upload
  ...
  }, [singleCompany, location.state]); // Add location.state to dependencies
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <Input
                type="text"
                name="name"
                value={input.name} // Set the value to controlled input
                onChange={changeEventHandler}
                required
              />
            </div>
