...
const Navbar = () => {
  ...
  const [me, setMe] = useState({ avatar: { url: "" }, role: "" });
  ...
  return (
    <>
      <div className="text-white z-20 fixed min-w-full bg-gray-950">
        <ul className="md:flex hidden justify-center items-center gap-24 pb-3 font-semibold text-xl">
          <Link
            to="/"
            className="flex fixed left-24 justify-center items-center titleT"
          >
            <MdOutlineBusinessCenter size={19} /> JOBFLOW
          </Link>
        ...
        >
          <ul className="bg-gray-950 bg-opacity-95 flex flex-col gap-20 text-2xl justify-start w-screen items-center">
            <Link
              onClick={() => setToggle(!toggle)}
              to="/"
              className="cool-link"
